...
var SettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    const start = (0, import_obsidian5.moment)();
    const end = (0, import_obsidian5.moment)().add(1, "hour");
    const previewText = this.plugin.settings.enableDateInserting ? `${end.format(this.plugin.settings.dateFormat)} ${end.format(
      this.plugin.settings.timeFormat
    )}` : `${end.format(this.plugin.settings.timeFormat)}`;
    const previewTextDoing = this.plugin.settings.enableDateInserting ? `${start.format(this.plugin.settings.dateFormat)} ${start.format(
      this.plugin.settings.timeFormat
    )}${this.plugin.settings.separator}${end.format(
      this.plugin.settings.dateFormat
    )} ${end.format(this.plugin.settings.timeFormat)}` : `${start.format(this.plugin.settings.timeFormat)}${this.plugin.settings.separator}${end.format(this.plugin.settings.timeFormat)}`;
    const desc = containerEl.createEl("p", {
      text: "Preview inserted datetime: ",
      cls: "inserted-preview"
    });
    desc.createEl("span", {
      text: this.plugin.settings.enableDoingStatus ? previewTextDoing : previewText
    });
  }
};
